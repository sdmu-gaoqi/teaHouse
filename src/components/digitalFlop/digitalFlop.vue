<template>
  <div class="digital-flop-component">
    ...
    >
      ...
      <div class="text-[20px] px-[5px]" v-if="index % 3 === 0 && index > 1">
        ,
      </div>
    </DigitalItem>
    <div class="text-[20px] px-[5px]">,</div>
    <DigitalItem
...
function initDigitalData(digitalValue: number) {
  ...
  if (props.maxLength && integerArray.length > props.maxLength) {
    console.error(`传入的数字${digitalValue}已经超过最大长度${props.maxLength}`)
  }
  // 添加千分位
